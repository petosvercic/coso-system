...
export function validateEditionJson(raw: string, existingSlugs: string[] = []) {
  ...
  try {
    ...
    return {
      ...
      debug: { rawStart: String(raw ?? "").slice(0, 120), normalizedStart: normalized.slice(0, 120) },
    };
  }

  if (!obj || typeof obj !== "object" || Array.isArray(obj)) return { ok: false as const, error: "NOT_OBJECT" };
  if (typeof obj.slug !== "string" || !obj.slug.trim()) return { ok: false as const, error: "MISSING_SLUG" };
  if (typeof obj.title !== "string" || !obj.title.trim()) return { ok: false as const, error: "MISSING_TITLE" };
  ...
  if (!/^[a-z0-9][a-z0-9-]{1,62}[a-z0-9]$/.test(slug)) return { ok: false as const, error: "BAD_SLUG", details: slug };
  if (existingSlugs.includes(slug)) return { ok: false as const, error: "DUPLICATE_SLUG", details: slug };
